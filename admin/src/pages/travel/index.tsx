...
interface ScenicSpotListResponse {
  list: ScenicSpotRecord[];
  total: number;
}
...
export default function TravelList() {
  ...
  const [data, setData] = useState<ScenicSpotRecord[]>([]);
  ...
  const fetchData = useCallback(async (p?: number, search?: string) => {
    ...
    setLoading(true);
    try {
      const res = await get<ScenicSpotListResponse>("/scenic-spots", {
        page: currentPage,
        pageSize: 20,
      ...
      message.error(err?.message || "加载票务列表失败");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: ColumnsType<ScenicSpotRecord> = [
    {
      title: "景区名称",
      dataIndex: "name",
      key: "name",
